...
<div class="contents" style="background:rgb(252, 250, 241)">
          ...
          <div class="breadcrumb-main">
            <div class="breadcrumb-action justify-content-center flex-wrap">
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <li class="breadcrumb-item active"><a href="viewmenu?category_id=<?php echo $category ?>"><i class="uil uil-arrow-left"></i>Go back to Stall</a></li>
                </ol>
              </nav>
            </div>
          </div>
  ...
  <div class="products mb-3">
    <div class="container-fluid">
      <div class="card product-details h-100 border-0" style="background:rgb(255, 249, 208)">
        <div class="product-item p-sm-50 p-20">
          <div class="row">
                ...
                <div class="product-item__body">
                  <div class="product-item__title">
                    ...
                    <div class="">
                      <?php
                      if ($item_status == 1) {
                        echo '<span class="px-2 badge-pill rounded-1 badge-default badge-danger">Not Available</span>';
                      } else {
                        echo '<span class="px-2 badge-pill rounded-1 badge-default badge-success">Available</span>';
                      }
                      ?>
                    </div>
                  </div>
                  <div class="product-item__content text-capitalize">
                    ...
                    <span class="product-desc-price text-dark">
                      Price: ₱
                      <?php
                      $sql_promo = "SELECT * FROM promo WHERE item_id = '$item_id' AND promo_status = 'active'";
                      $result_promo = $conn->query($sql_promo);
                      if ($result_promo->num_rows > 0) {
                        while ($rows_new = $result_promo->fetch_assoc()) {
                          echo '<span class="px-3 py-4">
                          <span class="badge-pill rounded position-absolute bg-warning top-0 end-0 py-3 px-2 text-white shadow-primary shadow-md">Promo</span>
                          ' . number_format($rows_new['item_current_price'], 2) . ' 
                          <span class="text-decoration-line-through text-danger mx-3">' . number_format($rows_new['item_promo_price'], 2) . '</span></span>';
                        }
                      } else {
            ...
            <div class="col-lg-12 w-100 d-flex justify-content-center my-lg-2">
              ...
              <button class="btn btn-squared border-0 me-10 my-sm-0 my-2 w-50 text-white"
                data-bs-toggle="modal" data-bs-target="#editModal<?php echo $item_id ?>" style="background:rgb(19, 39, 67)">Order</button>
                        ...
                        <div class="new-member-modal">
                              ...
                              <table class="w-100">
                                ...
                                <tbody class="text-dark">
                                  <tr>
                                    ...
                                    <td>
                                      <input type="number" id="quantity" class="form-control border-0 bg-white"
                                    ...
                                    <td class="text-end">
                                      <input type="text" class="form-control text-end border-0 bg-white"
                                        value="<?php
                                                ...
                                                $sql_up_price = "SELECT * FROM promo WHERE item_id = '$item_id' AND promo_status = 'active'";
                                                $res_up_price = $conn->query($sql_up_price);

                                                if ($res_up_price->num_rows > 0) {
                                                  while ($rows_p = $res_up_price->fetch_assoc()) {
                                                    echo $rows_p['item_current_price'];
                                                  }
                                                } else {
                          ...
                          <div class="card-footer d-flex justify-content-evenly align-items-center" style="background:rgb(255, 249, 208)"">
                            ...
                            <button type="submit" name="addtoorder"
                              class="btn btn-squared text-capitalize w-100 text-white" style="background:rgb(19, 39, 67)">Add to Order</button>
                          </div>
      ...
      <div class="row p-4">
        <?php
        $sql = "SELECT * FROM rating JOIN customer ON rating.customer_id = customer.customer_id where rating.item_id = '$item_id'";
