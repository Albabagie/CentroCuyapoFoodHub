...
if (isset($_POST['placeorder'])) {
    ...
    $stmt_check->bind_param("i", $order_numbers);
    $stmt_check->execute();
    $stmt_check->bind_result($existing_number);
    $stmt_check->fetch();
    $stmt_check->close();

    // If the generated order number exists, generate a new one
    while ($existing_number > 0) {
        $order_numbers = rand(111, 999);
        $stmt_check = $conn->prepare($sql_check_number);
        $stmt_check->bind_param("i", $order_numbers);
        $stmt_check->execute();
    ...
    $sql_order = "INSERT INTO over_orders (employee_id, over_date, over_number) VALUES (?,?, ?)";
    $stmt_order = $conn->prepare($sql_order);
    $stmt_order->bind_param("iss", $id, $sqldate, $order_numbers);

    if ($stmt_order->execute()) {
        $over_id = $stmt_order->insert_id;

        foreach ($_POST['product_id'] as $key => $item_id) {
        ...
        if ($stmt_total->execute()) {
            $update = "UPDATE otc o
                INNER JOIN otc_item oi ON oi.otc_id = o.otc_id
                SET o.otc_status = 1, oi.otc_order = 0
                WHERE o.otc_id IS NOT NULL;";
            ...
            if ($conn->query($update) === TRUE) {
                echo '<script>
                    document.addEventListener("DOMContentLoaded", function() {
                        ...
                        setTimeout(function() {
                            window.location.href = "otc.php";
                        }, 2000);
                    });
                </script>';
            } else {
                echo 'Update of otc and otc_item tables failed: ' . $conn->error;
            }
        } else {
        ...
        <div class="social-dash-wrap">
            ...
            <div class="row justify-content-center my-md-4">
                <div class="col-lg-12 justify-content-center">
                            ...
                            <tbody>
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        echo '<tr>';
                                        echo '<input type="hidden" value="' . $row['otc_id'] . '" name="product_id[]">';
                                        echo '<td><div class=""><input value="' . $row['item_name'] . '" name="order_name[]" class="border-0 w-100" readonly></div></td>';
                                        echo '<td>
                                        <div class="d-flex">
                            ...
                            <div class="card-footer rounded-bottom ">
                                <div class="d-flex justify-content-evenly">
                                    ...
                                    <div>
                                        ...
                                        <?php
                                        if ($total > 0): ?>
                                            <input id="total_price" value="<?php echo $total; ?>" class="row_total border-0 text-center" name="total" readonly>
                                        <?php else: ?>
                                            <input id="total_price" value="<?php echo 0; ?>" class="row_total border-0 text-center" name="total" readonly>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <?php
                  ...
                  <button type="submit" name="placeorder" class="btn btn-square btn-danger w-25" disabled >Place
                    Order</button>
                </div>';
                                } else {
                                    echo ' <div class="my-md-2 text-center d-flex justify-content-center ">
