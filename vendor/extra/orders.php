...
if (isset($_POST['placeorder'])) {
  ...
  $sql_order = "INSERT INTO orders (customer_id, order_date, order_number) VALUES (?, NOW(), ?)";
  ...
  if ($stmt_order->execute()) {
    ...
    if ($stmt_total->execute()) {
      ...
      if ($result_up === TRUE)
        $sql_item_up = "UPDATE cart_item SET ordered_item = 1 WHERE customer_id = '$id'";
      echo '<script>
                    document.addEventListener("DOMContentLoaded", function() {
                        Swal.fire({
                            title: "Order Sent.",
          ...
          <form method="POST">
            <div class="card ">
              ...
              <tbody>
                ...
                $sql = "SELECT * FROM cart c LEFT JOIN cart_item ci ON ci.product_id = c.product_id WHERE c.cart_status = 0 AND ci.item_void = 0 AND c.customer_id = '$id' GROUP BY c.customer_id, ci.product_id";
                ...
                if ($result->num_rows > 0) {
                  while ($row = $result->fetch_assoc()) {
                    ...
                    echo '<td><div class=""><input value="' . $row['name'] . '" name="order_name[]" class="border-0" readonly></div></td>';
                    echo '<td><div><input value="' . $row['qty'] . '" type="number" class="border text-center quantity_input" name="order_qty[]" id="qty" min="1" max="50" data-price="' . $row['price'] . '">
                    </div></td>';
                    echo '<td><div><input value="' . $row['price'] . '" type="number" class="border-0 text-center price_input" name="price[]" readonly></div></td>';
                    echo '<td><div>';
